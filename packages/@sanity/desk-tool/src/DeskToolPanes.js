...
export default class DeskToolPanes extends React.Component {
  ...
  renderPanes() {
    ...
    return flatten(
      panes.map((paneDef, i) => {
        const paneDefs = Array.isArray(paneDef) ? paneDef : [paneDef]
        return paneDefs.map((pane, sub) => {
          const hasMultiplePanes = paneDefs.length > 1
          const isCollapsed = this.state.collapsedPanes.includes(i)
          const paneKey = `${i}-${keys[i - 1] || 'root'}-${sub}`

          // Same pane might appear multiple times, so use index as tiebreaker
          const wrapperKey = pane === LOADING ? `loading-${i}-${sub}` : `${i}-${pane.id}-${sub}`
          path.push(pane.id || `[${i}]`)
          ...
          return (
            <SplitPaneWrapper
              key={wrapperKey}
              minWidth={70}
              defaultWidth={hasMultiplePanes ? 600 : 300}
