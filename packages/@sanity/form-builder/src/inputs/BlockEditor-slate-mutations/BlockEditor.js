...
import {State, Editor, setKeyGenerator} from 'slate'
...
export default class BlockEditor extends React.Component {
  ...
  hasBlock(field) {
    const {value} = this.props
    return value.blocks.some(node => node.type === field.type
      && isEqual(
  ...
  handleSetMark = event => {
    this.props.onChange(this.props.value
      .transform()
      .addMark(event.currentTarget.dataset.mark)
      .apply())
  }
  ...
  render() {
    ...
    const activeLink = null // this.getActiveLink()
    const showLinkButton = false //(value.selection && value.selection.isExpanded) || !!activeLink

    return (
      <FormField label={field.title} level={level}>
        <div
          className={`
          ...
          `}
        >
          <button onClick={this.handleSetMark} data-mark="strong">Bold</button>
          <Editor
